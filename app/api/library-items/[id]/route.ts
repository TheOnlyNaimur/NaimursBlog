...
export async function PUT(
  ...
  try {
    const { id } = await params;
    const existingItem = await prisma.libraryItem.findUnique({ where: { id } });
    ...
    const duplicateItem = await prisma.libraryItem.findFirst({
      where: {
        slug,
        NOT: { id },
      },
    ...
    if (duplicateItem) {
      return NextResponse.json(
        { message: "Another library item already uses this slug." },
        { status: 409 }
      );
    }

    const status = body.status === "published" ? "PUBLISHED" : "DRAFT";
    const publishedAt = status === "PUBLISHED" ? existingItem.publishedAt ?? new Date() : null;
    ...
    return NextResponse.json(
      { message: "Something went wrong while updating the library item." },
      { status: 500 }
    );
  }
}
